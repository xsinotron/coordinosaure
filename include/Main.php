...
///////////
    public function getList($list) {
        switch ($list) {
            case 'amapiens':
                ...
                echo $this->model->gen(
                    'include/views/tables/amapiens.tpl',
                    new Amapiens($liste)
                );
                break;
            case 'producteurs':
                ...
                echo $this->model->gen(
                    'include/views/tables/producteurs.tpl',
            ...
            case 'contrats':
                ...
                echo $this->model->gen(
                    'include/views/tables/contrats.tpl',
                    new Contrats($liste)
                );
                break;
    ...
    function popin ($mode, $req) {
        ...
        switch ($mode) {
            case 'producteur':
                $form = $this->model->gen(
                'include/views/forms/producteur.tpl',
                new Producteur(
                array("mode" => $req)
                )
                );
            ...
            case 'contrat':
                $form = $this->model->gen(
                'include/views/forms/contrat.tpl',
                new Contrat(
                array(
                "mode"     => $req,
            ...
            case 'contrat_type':
                $form = $this->model->gen(
                'include/views/forms/contrat_type.tpl',
                new ContratType(
                array(
                "mode"     => $req,
                "producteurs" => $this->model->getProducteurs()
            ...
            case 'produit':
                $form = $this->model->gen(
                'include/views/forms/produit.tpl',
                new Produit(
                array(
                "mode"     => $req,
            ...
            default:
                $form = $this->model->gen(
                'include/views/forms/amapien.tpl',
                new Amapien($data)
                );
    ...
    public function mainMenu ($page) {
        return $this->model->gen("include/views/mainMenu.tpl", new MainMenu($page));
    }
